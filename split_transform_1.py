...
class PrintGrad(Function):
    @staticmethod
    def forward(ctx, x):
        # print("forward type: ", type(x))
        return 1 * x
    @staticmethod
    def backward(ctx, grad_output):
        print("grad backward: ", torch.sum(torch.abs(grad_output.data)))
        return grad_output

printgrad = PrintGrad.apply
...
def train_model(model, clscriterion, dmncriterion, srcoptimizer, taroptimizer,
    ...
    for epoch in range(num_epochs):
        ...
        for data in srcdata:
            ...
            turn = random.randint(0,1) # randomly choose to pass source or target

            def train_src():
                nonlocal running_clsloss
                nonlocal running_dmnloss
...
class GRLModel(nn.Module):
    def __init__(self):
        ...
        self.tDmn = nn.Sequential(nn.Linear(512,64), nn.SELU(inplace=True),
                                       nn.Linear(64,512), nn.SELU(inplace=True))
        self.tCls = nn.Sequential(nn.Linear(512,256), nn.SELU(inplace=True),
                                       nn.Linear(256,512), nn.SELU(inplace=True))
        self.is_source = True
        self.grl = nn.Sequential(
            nn.Linear(512,64), nn.SELU(inplace=True),
            nn.Linear(64,2)
            )
