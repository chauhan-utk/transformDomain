...
class STensor(Function):
    @staticmethod
    def forward(ctx, x):
        return x, x.clone()
    @staticmethod
    def backward(ctx, grad_x1, grad_x2):
        grad_x1.data += grad_x2.data
        # print("grad backward: ", torch.sum(torch.abs(grad_x1.data)))
        return grad_x1

split_tensor = STensor.apply
    ...
    def forward(self, x):
        if self.training:
            ...
            if not self.is_source:
                x = self.transform(x)
            x1, x2 = split_tensor(x) # to handle backward nicely
            x1 = grl(x1)
