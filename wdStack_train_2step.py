...
from wdStackDomain import WDDomain
...
{'params' : model_ft.features.basenet.fc.parameters(), 'lr' : 1e-3, 'betas' : (0.5, 0.9), 'weight_decay' : 1e-5} 
]
    ...
    for epoch in range(num_epochs):
        ...
        for data in srcdata:
            ...
            # if X.is_cuda:
                # eps = eps.cuda()
            # eps = Variable(eps)
            X_inter = X.cpu().data + eps * (G.cpu().data - X.cpu().data)
            X_inter = Variable(X_inter, requires_grad=True)
            if X.is_cuda:
                X_inter = X_inter.cuda()
            ...
            d_loss = (d_l + gp_loss * gp_lambda) * lmbda
