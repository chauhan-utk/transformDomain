...
class GRLModel(nn.Module):
    def __init__(self):
        ...
        self.tDmn = nn.Sequential(nn.Linear(512,512), nn.ReLU(inplace=True),
                                       nn.Linear(512,512), nn.ReLU(inplace=True))
        self.tCls = nn.Sequential(nn.Linear(512,512), nn.ReLU(inplace=True),
                                       nn.Linear(512,512), nn.ReLU(inplace=True))
        self.is_source = True
...
src_params += list(model_ft.tCls.parameters())
srcoptimizer = optim.SGD(src_params, lr=0.01, momentum=0.9, weight_decay=1.)
param_group = []
param_group += list(model_ft.features.parameters())
param_group += list(model_ft.tDmn.parameters())
param_group += list(model_ft.grl.parameters())
taroptimizer = optim.SGD(param_group, lr=0.01, momentum=0.9, weight_decay=1.)
